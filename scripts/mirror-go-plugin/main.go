...
func main() {
	...
	err := filepath.Walk(dir, func(path string, info fs.FileInfo, err error) error {
		...
		contents = bytes.ReplaceAll(contents,
			[]byte(`"github.com/EnduIf/sqlc/internal/codegen/golang/opts"`),
			[]byte(`"github.com/EnduIf/sqlc-gen-go/internal/opts"`))

		contents = bytes.ReplaceAll(contents,
			[]byte(`"github.com/EnduIf/sqlc/internal/plugin"`),
			[]byte(`"github.com/sqlc-dev/plugin-sdk-go/plugin"`))

		contents = bytes.ReplaceAll(contents,
			[]byte(`"github.com/EnduIf/sqlc/internal/codegen/sdk"`),
			[]byte(`"github.com/sqlc-dev/plugin-sdk-go/sdk"`))

		contents = bytes.ReplaceAll(contents,
			[]byte(`"github.com/EnduIf/sqlc/internal/metadata"`),
			[]byte(`"github.com/sqlc-dev/plugin-sdk-go/metadata"`))

		contents = bytes.ReplaceAll(contents,
			[]byte(`"github.com/EnduIf/sqlc/internal/pattern"`),
			[]byte(`"github.com/sqlc-dev/plugin-sdk-go/pattern"`))

		contents = bytes.ReplaceAll(contents,
			[]byte(`"github.com/EnduIf/sqlc/internal/debug"`),
			[]byte(`"github.com/EnduIf/sqlc-gen-go/internal/debug"`))

		contents = bytes.ReplaceAll(contents,
			[]byte(`"github.com/EnduIf/sqlc/internal/inflection"`),
			[]byte(`"github.com/EnduIf/sqlc-gen-go/internal/inflection"`))
